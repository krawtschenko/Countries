...
export const Info: React.FC<InfoType> = (props) => {
    const {
        name, nativeName, flags, capital, population, region, subregion, topLevelDomain, currencies = [],
        languages = [], borders = [], push
    } = props;
    ...
    useEffect(() => {
        if (borders.length)
            axios
                ...
                .then(({data}) => {
                    setNeighbors(data.map((c: Country) => c.name))
                });
    }, [borders]);
